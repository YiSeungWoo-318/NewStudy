...
y_data = tf.one_hot(y_data, depth=3).eval()
...
def loss(y):
    tf.reduce_mean(-tf.reduce_sum(y * tf.compat.v1.log(h), axis=1))
...
def opt():
    tf.optimizers.SGD(learning_rate=2e-2)
...
for step in range(2001):
    _, h_val, cost_val = opt, h(x_train), loss(y_train)
...
def pred(y):
    tf.equal(tf.argmax(h, 1), tf.argmax(y, 1))

def acc():
    tf.reduce_mean(tf.cast(pred, dtype=tf.float32))
